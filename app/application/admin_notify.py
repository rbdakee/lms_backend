...
Флаги `notify_leads`, `notify_submissions` и `notify_certificates` — настройка
площадки, поэтому решение «слать или нет» живёт здесь, а не в адаптере: иначе
каждый следующий адаптер переписывал бы эту проверку заново, а переключатель
на экране врал бы до тех пор, пока его кто-нибудь не забудет.
...
# Флаг настройки на каждый тип сообщения — их ровно три (BACKEND_NOTES, 11):
# третьим стала заявка на сертификат (CERTIFICATES_BRIEF, 3)
NOTIFY_FLAGS = {
    "lead": "notify_leads",
    "submission": "notify_submissions",
    "certificate": "notify_certificates",
}
...
class AdminNotifier:
    ...
    def notify_admins(self, card: NotificationCard, *, kind: str, platform: str) -> None:
        ...
        (PLATFORMS_BRIEF, решение 6), и забыть площадку в очередной отправке —
        третья приехала с заявкой на сертификат — отсюда невозможно.
        ...
        # работы на проверку и просьбы о сертификате
