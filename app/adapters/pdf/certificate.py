...
def _place_image(
    ...
    Настройки пропускают любой `image/*`, включая svg, который fpdf2 может
    не взять, а файл в хранилище бывает и битым. Отсутствие печати не повод
    отказать человеку в сертификате, поэтому неудача вставки гасится здесь:
    в лог уходит слот и сам факт, без байтов и без ФИО (правило ПД).
    """
    if data is None:
        return
    try:
